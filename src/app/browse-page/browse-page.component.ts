...
export class BrowsePageComponent {
      ...
      if (this.searchType == 'topic') {
        this.matchedTopics = this.cards
          ...
          .filter((topic: string, index: number, self: string[]) =>
            self.indexOf(topic) === index && // Ensure no duplicates
            topic.toLowerCase().includes(this.searchTerm.toLowerCase()) // Filter by the search term
          );
      }
  ...
  //this method handles the case where a user cliks on a topic from the search bar
